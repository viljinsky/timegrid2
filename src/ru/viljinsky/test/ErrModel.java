        ...
        for (TableProp prop:list){
            ...
            try{
                for (Dataset ds1:ds.getForeignDataset()){
                    System.out.println("XX"+ds1.getTableName());
                    TableProp prop2 = findProp(ds1);
                    if (prop2!=null){
                        links.add(new Link(prop, prop2));
//                        prop.referenced.add(prop2);
                    }
...
public class ErrModel extends JFrame {
    ...
    public void open(){
        try{
            setTitle(dataModule.getDataName());
            firstOperation();
        } catch (Exception e){
    ...
    public void firstOperation() throws Exception{
        ...
        try{
            ...
            throw new Exception("Ошибка при выполнении firstOperation"+e.getMessage());
        }
    }
